...
1. Получить данные за 2012 год ([example_start.py](example_start.py)).

2. Выбрать крупную компанию, найти на ее сайте отчетность по РСБУ за 2012 год,
   сравнить данные с сайта и из базы отчтености Росстата.  

3. Проверить правильность составления отчетности через равенства (например, `ta = tp`
   и неравенства между переменными (например, `debt_long < tp_long`.
   Добавление: доработать `test_dataset.py` c учетом `variables.py`.

4. Вывести список крупнейших российских компаний по объему активов и по 
...
- Определите пары переменных в отчетности и как они они связаны между собой. 
  При каких условиях? Покажите на графиках.  
